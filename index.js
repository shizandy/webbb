...
app.post('/login', (req, res) => {
    ...
    fs.readFile(akunFilePath, 'utf-8', (err, data) => {
        if (err) {
            ...
            return res.status(500).json({ error: 'An error occurred while loading account data.' });
        }
        try {
            ...
            if (akun) {
                ...
                return res.redirect('/login?error=Username%20or%20password%20incorrect!');
            }
        } catch (parseError) {
            console.error("Error parsing JSON", parseError);
            return res.status(500).json({ error: 'An error occurred in parsing account data.' });
        }
    });
});
...
app.post('/create-panel', async (req, res) => {
    ...
    switch (ramOption) {
        ...
        default:
            return res.status(400).json({ message: "❌ Invalid RAM selection!" });
    }
